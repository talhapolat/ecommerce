	...
	<div class="container-menu-desktop">
		...
		<div class="wrap-menu-desktop">
			<nav class="limiter-menu-desktop container">
				...
				<div class="menu-desktop">
					<ul class="main-menu">
						<!-- <li class="active-menu"> -->
							...
							<?php 
							$navQuery = $dbConnect->prepare("SELECT * FROM navigation WHERE main_nav is null");
							$navQuery->execute();
							$navNum = $navQuery->rowCount();
							$navs = $navQuery->fetchAll(PDO::FETCH_ASSOC);
							?>

							<?php foreach ($navs as $key => $nav): 
								$navSubQuery = $dbConnect->prepare("SELECT * FROM navigation WHERE main_nav = ? ORDER BY number");
								$navSubQuery->execute([$nav['id']]);
								$navSubNum = $navSubQuery->rowCount();
								$navSubs = $navSubQuery->fetchAll(PDO::FETCH_ASSOC);
								?>
								...
								<?php if ($navSubNum > 0): ?>
									<li>
										...
										<ul class="sub-menu">
											<?php foreach ($navSubs as $key => $navSub): ?>
												<?php if ($navSub['image'] == null): ?>
													<li style="border-bottom: 1px solid #cbcac3;"><a href="<?= $navSub['url'] ?>" style="color: <?= $navSub['color'] ?>;"><?= $navSub['title'] ?></a></li>
												<?php else: ?>
													<li style="border-bottom: 1px solid #cbcac3;">
														<a style="margin:0px;padding: 0px;width: auto;height: auto; color: <?= $navSub['color'] ?>;">
															<div onclick="window.location='<?= $navSub['url'] ?>';" style="cursor: pointer;background: #474745 url(<?= $navSub['image'] ?>) no-repeat; height: 100px; display:flex; justify-content:center;align-items: center;">
																<div style="font-family: Poppins-Regular;"><?= $navSub['title'] ?></a></div>
															</div>
														</a>
													</li>
												<?php endif ?>
								...
								<?php else: ?>
									<li>
										<a href="<?= $nav['url'] ?>" style="color: <?= $nav['color'] ?>;"><?= $nav['title'] ?></a>
									</li>
								<?php endif ?>
							<?php endforeach ?>
				...
				<a href="/favorites" class="dis-block icon-header-item cl2 hov-cl1 trans-04 p-l-22 p-r-11 icon-header-noti" data-notify="<?= $productNum ?>">
...
<div class="wrap-header-mobile">
	...
	<div class="wrap-icon-header flex-w flex-r-m m-r-15">
		...
		<div id="cartnotimobil" class="icon-header-item cl2 hov-cl1 trans-04 p-r-11 p-l-10 icon-header-noti js-show-cart" data-notify="<?php if (isset($_SESSION['cart'])) {
